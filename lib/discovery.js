...
Discovery.probe = function(options, callback) {
	if (callback === undefined) {
		if (typeof options === 'function') {
			callback = options;
			options = {resultType: 'cam'};
		} else {
			options = options || {resultType: 'cam'};
		}
	}
	...
	const listener = function(msg, rinfo) {
		...
		const fmtErr = !((data) &&
			  (data.probeMatches) &&
			  (data.probeMatches.probeMatch) &&
			  (data.probeMatches.probeMatch.XAddrs));
			  		
			// TODO check for matching RelatesTo field and messageId
			if (err || fmtErr) {
				const errMsg = 'Wrong SOAP message from ' + rinfo.address + ':' + rinfo.port;
				errors.push(err || new Error(errMsg, xml));
				/**
				 * Indicates error response from device.
				 * @event Discovery#error
				 * @type {string}
				 */
				Discovery.emit(errMsg, xml);
			} else {
				...
				data.probeMatches.probeMatch.XAddrs.split(' ').forEach(
					function(camAddr) {													
						if (!cams[camAddr]) {
							var cam;
