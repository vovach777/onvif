...
Cam.prototype.connect = function(callback) {
    var _this = this;
    this.getSystemDateAndTime( function(err,camDate,xml) {        
        if (err) {
           if (callback) {
	...
	console.log('cam Time: ' + camDate.toISOString());
        _this.timeShift =  camDate.getTime()-Date.now();
	_this._connect(callback);
  });
};


Cam.prototype._connect = function(callback) {
...
Cam.prototype.getSystemDateAndTime = function(callback) {
	this._request({
		...
		callback.call(this, err, err ? null : new Date(Date.UTC(dt.date.year, dt.date.month - 1, dt.date.day, dt.time.hour, dt.time.minute, dt.time.second)), xml);
	});
};
...
Cam.prototype._passwordDigest = function() {
	var timestamp = (new Date( Date.now() + (this.timeShift||0))).toISOString();
        ...
        //console.log( 'pas Time: ' + timestamp);
