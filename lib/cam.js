...
Cam.prototype.connect = function(callback) {
	var upstartFunctions = [this.getSystemDateAndTime, this.getCapabilities, this.getDeviceInformation, this.getProfiles, this.getVideoSources]
	...
	if (!callback) 
	  callback = emptyFn;  
    function shift() {					
		var fun = upstartFunctions.shift();
		if (fun) {
		fun.call(this,(function(a) {
			if (a) 
...
Cam.prototype._request = function(options, callback) {
	...
	reqOptions.headers = {
		'Content-Type': options.action ? 'application/soap+xml; charset=utf-8; action="'+options.action+'"' : 'application/soap+xml; charset=utf-8'
...
Cam.prototype.getSystemDateAndTime = function(callback) {
	this._request({
		...
		if (!err) {
			...
			this.time = new Date(Date.UTC(dt.date.year, 
								dt.date.month - 1, 
								dt.date.day, 
								dt.time.hour, 
								dt.time.minute, 
								dt.time.second));
			console.log('cam  time:' +  this.time.toISOString() );
			if (!this.timeShift) {
			  this.timeShift = this.time-Date.now();
			  console.log('shift:' +  this.timeShift );
			}
		} else
		{
			console.error('time err');
		}
		callback.call(this, err, err ? null : this.time , xml);
	}.bind(this));
};
